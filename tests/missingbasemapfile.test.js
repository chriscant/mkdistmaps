...
test('CHECK MESSAGE WHEN NO BASEMAP FILE GIVEN', async () => {
  ...
  const argv = ['node', '.', 'tests/data/missingbasemapfile-test-config.json']
  await mkdistmaps.run(argv)
  expect(lastOutput).toBe('No basemap file given')
})
